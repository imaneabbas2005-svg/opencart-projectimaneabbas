...
class ControllerCatalogCategory extends Controller {
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_parent_path'])) {
			$filter_parent_path = $this->request->get['filter_parent_path'];
		} else {
			$filter_parent_path = null;
		}
		...
		$data = array(
			...
			'filter_parent_path'   => $filter_parent_path,
			'sort'                 => $sort,
			'order'                => $order,
		...
		$category_total = $this->model_catalog_category->getTotalCategoriesFiltered($data);
		...
		foreach ($results as $result) {
			...
			$product_count = $this->model_catalog_category->getTotalProductsByCategoryId($result['category_id']);
			$parent_path = $this->model_catalog_category->getCategoryParentPath($result['category_id']);
		...
		$this->data['button_reset'] = 'Reset Filter';
		...
		if (isset($this->request->get['filter_parent_path'])) {
			$url .= '&filter_parent_path=' . $this->request->get['filter_parent_path'];
		}
		if (isset($this->request->get['page'])) {
		...
		$this->data['sort_product_count'] = $this->url->link('catalog/category', 'token=' . $this->session->data['token'] . '&sort=product_count' . $url, 'SSL');
		...
		$this->data['filter_parent_path'] = $filter_parent_path;

		$this->data['all_categories'] = $this->model_catalog_category->getCategoriesForFilter();
